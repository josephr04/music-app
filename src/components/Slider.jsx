...
export function VolumeSlider({ value = 0, onChange, max = 100 }) {
  const [sliderValue, setSliderValue] = useState(value);
  const [isDragging, setIsDragging] = useState(false);
  const [isHovered, setIsHovered] = useState(false);
  const sliderRef = useRef();

  // Updates the value when it's changed  (prop `value`)
  useEffect(() => {
    setSliderValue(value);
  }, [value]);
  ...
  return (
    ...
    >
      {/* Background bar */}
      <div
        className="absolute w-full h-[2px] bg-slate-400 rounded-md"
      ></div>

      {/* Progress bar */}
      <div
        className={`absolute h-[2px] bg-white rounded-md`}
        style={{ width: `${(sliderValue / max) * 100}%` }}
      ></div>
